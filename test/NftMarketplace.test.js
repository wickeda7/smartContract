    ...
    : describe("Nft Marketplace Unit Tests", function () {
          ...
          describe("Mint and list a new NFT token", function () {
              ...
              it("Should emit MarketItemCreated after successfully listing of NFT", async function () {
                  ...
                  await expect(transaction)
                      ...
                      .withArgs(contractOwner.address, nftMarketAddress, tokenID, auctionPrice)
              })
          })
          describe("Execute sale of a marketplace item", function () {
              ...
              it("Should revert if auction price is not correct", async () => {
                  ...
                  await expect(
                      nftMarket.connect(buyerAddress).createMarketSale(newNftToken, { value: 20 })
                  ).to.be.revertedWith(
          ...
          describe("Resale of a marketplace item", function () {
              ...
              it("Should revert if the token owner or listing price is not correct", async () => {
                  ...
                  await nftMarket
                      ...
                      .createMarketSale(newNftToken, { value: auctionPrice })
                  await expect(
                      nftMarket.resellToken(newNftToken, auctionPrice, { value: listingPrice })
              ...
              //   it("Should emit item bought", async () => {
              //       const transaction = await nftMarket.createToken(tokenURI, auctionPrice, {
              //           value: listingPrice,
              //       })
              //       const receipt = await transaction.wait()
              ...
              //       await nftMarket
              //           .connect(buyerAddress)
              //           .createMarketSale(tokenID, { value: auctionPrice })

              //       const tokenOwnerAddress = await nftMarket.ownerOf(tokenID)

              //       await nftMarket
              //           .connect(buyerAddress)
              //           .resellToken(tokenID, auctionPrice, { value: listingPrice })
              //       const newTokenOwner = await nftMarket.ownerOf(tokenID)
              //       console.log(newTokenOwner)
              //       console.log(nftMarketAddress)
              //       await expect(transaction)
              //           .to.emit(nftMarket, "ItemBought")
              //           .withArgs(newTokenOwner, newTokenOwner, tokenID, auctionPrice)
              //   })
          })
